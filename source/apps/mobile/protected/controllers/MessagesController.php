...
class MessagesController extends MemberController
{
	public function actionFriends() {
		...
		$friends = Yii::app()->user->listOnline();
		
		$this->render('page/friends', array(
			'friends' => $friends,
		));
	}
	public function actionIndex() {
		
		// header('Location: http://m.plun.asia');
	    ...
	    if(Yii::app()->request->isAjaxRequest) {
	    	$this->renderPartial('page/index', array(
	    			'page'=>$page,
	    			'limit'=>$limit,
	    			'messages'=>$messages,
	    	));
	...
	public function actionCw($u) {
		$username = Util::decrypt($u);
		
		if($user = Member::model()->find('username = :username', array(':username'=>$username))) {
	...
	public function actionUpdateOffline() {
		if (Yii::app()->request->isAjaxRequest && Yii::app()->request->isPostRequest){
			$username = Yii::app()->request->getPost('jid');
			
			if(!Member::model()->find('username = :username', array(':username'=>$username)))
				exit();
			
			OfflineMessages::model()->updateOffline(Yii::app()->user->id, $username, TRUE);
			
			echo Util::encrypt($username);
		}
	}
...
// 	                        $results['msg'] = CFormModelHelper::model()->parseErrorToString($msgFrm);
	...
	public function actionReply()
	{
	    if (Yii::app()->request->isAjaxRequest && Yii::app()->request->isPostRequest){
	        ...
	        if(!empty($answer_id) && !empty($to) && !empty($from)){
	...
	public function actionView($id = null){
        $model = new Message();
		if(isset($id)){	   
            $pk = Util::decryptRandCode($id);
            $model = Message::model()->findByPk($pk);            
	        $this->render('partial/view', array('model'=>$model));
	    } else {
		    throw new CHttpException ( 404, 'The requested page does not exist.' ); 
	...
	public function actionCompose(){
		$model = new MessageForm();
		$receiver	=	Yii::app()->request->getParam('receiver', false);
		$this->render('page/compose', array('model' => $model, 'receiver' => $receiver));
	}
	
	public function beforeAction($action) {
		if( parent::beforeAction($action) ) {
			$cs = Yii::app()->clientScript;
			$cs->registerScriptFile(Yii::app()->theme->baseUrl . '/resources/js/messages/common.js', CClientScript::POS_BEGIN);
			return true;
		}
		return false;
	}
