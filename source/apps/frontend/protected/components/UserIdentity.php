...
class UserIdentity extends CUserIdentity {
	...
	public function authenticateFacebook() {
		...
		$user = ($profile) ? Member::model()->findByPk($profile->user_id) : null;
		if ($user === null)
			$this->errorCode = self::ERROR_USERNAME_INVALID;
		else if($user->status == Member::STATUS_INACTIVE)
			$this->errorCode = self::ERROR_STATUS_INACTIVE;
		else if($user->status == Member::STATUS_BANNED)
...
// 			$this->errorCode=self::ERROR_STATUS_INACTIVE;
			...
			$this->errorCode=self::ERROR_STATUS_BANNED;
		else if($user->status == Member::STATUS_REMOVED)
