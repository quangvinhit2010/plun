...
class SettingsController extends MemberController {
	...
	public function actionExtra(){
		...
		$this->render('partial/extra', array(
				'model' => $model,
				'model_profile' => $model_profile,
		));		
	}
	public function actionLanguages(){
		...
		$this->render('partial/language_settings', array(
				'model' => $model,
				'model_profile' => $model_profile,
		));
	}
	public function actionChangepass(){
		...
		$this->render('partial/changepass', array(
				'model' => $model,
				'model_profile' => $model_profile,
		));
	}		
	...
	public function actionSave() {
		...
		if (Yii::app()->request->isAjaxRequest && Yii::app()->request->isPostRequest) {
			...
			//for my types
			$my_types	=	isset($post['my_types'])		?	$post['my_types']	:	false;
			if($my_types){
				$post['my_types'] = implode(',', $my_types);
			}else{
				$post['my_types'] = '';
			}
			...
			//for into_stuff
			$into_stuff	=	isset($post['into_stuff'])		?	$post['into_stuff']	:	false;
			if($into_stuff){
				$post['into_stuff'] = implode(',', $into_stuff);
			}else{
				$post['into_stuff'] = '';
			}
			...
			if(!YumEncrypt::validate_password($user_post['password'], $model_member->password, $model_member->salt)){
				...
				if(!empty(CParams::load()->params->vtn->migrate->updateuser)){
					$return = VTN::model()->updateUser(array(
						'email'=>$post['email'],
						'password'=>$user_post['new_password'],
						'username'=>$model_member->username,
					));
				}
				/**end**/
			}
	...
	public function actionLanguageSettings(){
		...
		if (Yii::app()->request->isAjaxRequest && Yii::app()->request->isPostRequest) {
			...
			if(!empty($usr_setting->measurement) && $usr_setting->measurement != $usr_setting_post['measurement']){
    			$usr_setting->height = UnitHelper::model()->convert('height', $usr_setting->height, $usr_setting_post['measurement']);
    			$usr_setting->weight = UnitHelper::model()->convert('weight', $usr_setting->weight, $usr_setting_post['measurement']);
			}
	...
	public function actionAvatar(){
	    $this->renderPartial('partial/settings_avatar', array(
	    ));	    
	}
}