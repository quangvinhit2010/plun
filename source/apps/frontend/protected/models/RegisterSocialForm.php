 ...
 */
class RegisterSocialForm extends CFormModel
{
	
	const COUNTRY_ID_VN = 230;
	...
	public $openid;
	public $openid_type;
	public $openid_identification;
	public $openid_register_complete;
	...
	public function rules()
	{
		...
		return array(
			...
			array('username, email, lastname, firstname, password', 'required',
	        ...
	        array('username, email, lastname, firstname, day, month, year, password, openid, openid_type, openid_identification, openid_register_complete', 'safe'),
		);
	}
	...
	public function validation($value){
		...
		/**validate vtn**/
		if(!empty(CParams::load()->params->vtn->migrate->validate)){
			$data = array(
					'username' => $this->username,
					'email' => $this->email,
			);
			$return = VTN::model()->validate($data);
			if(!empty($return->response->errormessage[0])){
				switch ($return->response->errormessage[0]){
					case 'usernametaken':
						$msg = Lang::t('register', "Someone already has that username. Please try another!");
	...
	public function save()
	{
		...
		if(!empty($this->email)){
			...
			$user->openid_type = $this->openid_type;
			$user->openid_identification = $this->openid_identification;
			$user->openid_register_complete = $this->openid_register_complete;
			...
			if($user->save()) {
				if(Yum::hasModule('profile')) {
					...
					if($profile->save()){
						...
						//
						$country_id	=	isset($current_country['id'])	?	$current_country['id']	:	RegisterForm::COUNTRY_ID_VN;
						if(!$country_id){
							$country_id	=	RegisterForm::COUNTRY_ID_VN;
						} 
					    ...
					    */
					    ...
					    //begin save referrer
				    	$cookie = Util::getCookie('referrer');
					    if(isset($cookie)){
					    	$referrer_id = ReferrerDefine::model()->findByAttributes(array('domain' => $cookie['domain']));
					    	if(isset($referrer_id->id)){
						    	$referrer = new ReferrerLog();
						    	$referrer->type_referrer = $cookie['type_referrer'];
						    	$referrer->redirect_url = $cookie['redirect_url'];
						    	$referrer->referrer_url = $cookie['referrer_url'];
						    	$referrer->referrer_id = $referrer_id->id;
						    	$referrer->user_id = $user->id;
						    	$referrer->type_log = ReferrerLog::TYPE_LOG_REGISTER;
						    	$referrer->created = time(); 
					    		$referrer->save();
						    	Util::writeCookie(array('name' => 'referrer', 'value' => '', 'valueDefault' => serialize(array())));
