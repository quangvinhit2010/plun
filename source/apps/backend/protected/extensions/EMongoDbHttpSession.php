...
class EMongoDbHttpSession extends CHttpSession {
	...
	public function init() {
		$this->collection = YumSession::model()->getCollection();
		$this->options = array ('fsync' => $this->fsync, 'safe' => $this->safe );
		
		$this->collection->ensureIndex(array('_id' => true));
    	$this->collection->ensureIndex(array('expire' => true));
	...
	public function writeSession($id, $data) {
		$opts = $this->options;
		$opts ['upsert'] = true;
		return $this->collection->update ( array ($this->idColumn => $id ), array ($this->dataColumn => $data, $this->expireColumn => $this->getExipireTime (), $this->idColumn => $id ), $opts );
	}
	...
	public function destroySession($id) {
		return $this->collection->remove ( array ($this->idColumn => $id ), $this->options );
	}
	...
	public function gcSession($maxLifetime) {
		return $this->collection->remove ( array ($this->expireColumn => array ('$lt' => time () ) ), $this->options );
	}
	...
	public function regenerateID($deleteOldSession = false) {
		...
		if (is_null ( $row )) {
			$this->collection->insert ( array ($this->idColumn => $newId, $this->expireColumn => $this->getExipireTime () ), $this->options );
		} else if ($deleteOldSession) {
			$this->collection->update ( array ($this->idColumn => $oldId ), array ($this->idColumn => $newId ), $this->options );
		} else {
