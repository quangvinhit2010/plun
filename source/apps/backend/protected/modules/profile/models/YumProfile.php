...
class YumProfile extends YumActiveRecord {
    ...
    public function getPublicFields() {
        ...
        if ($privacy = YumUser::model()
            ...
            foreach (YumProfileField::model()->cache(3600)->findAll() as $field) {
                if (
                        (($i & $privacy) && $field->visible != YumProfileField::VISIBLE_HIDDEN) || $field->visible == YumProfileField::VISIBLE_PUBLIC)
                    $fields[] = $field;
                $i*=2;
            }
        }
	...
	public function checkEmailExist($current_email, $new_email){
		$criteria = new CDbCriteria();
		$criteria->condition = "email <> '{$current_email}' AND email = '{$new_email}'";
