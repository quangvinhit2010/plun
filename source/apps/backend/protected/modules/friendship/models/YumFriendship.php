...
class YumFriendship extends YumActiveRecord {
	...
	public function acceptFriendship() {
		...
		if(Yum::hasModule('message') 
			...
			Yii::import('backend.modules.message.models.YumMessage');
	...
	public function relations()
	{
		return array(
				...
				'invited' => array(self::BELONGS_TO, 'Member', 'friend_id'),
                                'profile_settings' => array(self::BELONGS_TO, 'UsrProfileSettings', 'inviter_id'),
				);
	}
...
// 									'{message}' => $this->message)));
	...
	public static function areFriends($uid1, $uid2) {
		if(is_numeric($uid1) && is_numeric($uid2)) {
			$friendship = YumFriendship::model()->cache(500)->find('status = 2 and inviter_id = '.$uid1 . ' and friend_id = '.$uid2);
			if($friendship)
				return true;

			$friendship = YumFriendship::model()->cache(500)->find('status = 2 and inviter_id = '.$uid2 . ' and friend_id = '.$uid1);
			if($friendship)
				return true;
		} 
		return false;
	...
	public static function getFriendList($inveter_id){
		if(isset($inveter_id)){
			return YumFriendship::model()->findAll('inviter_id = '.$inveter_id);
		} else {
			return false;
		}		
		
	}
	public function searchFriendList($keyword, $my_id){
		$criteria=new CDbCriteria;
		$criteria->select = "u.id friend_id, u.username AS friend_name";
		$criteria->distinct	=	true;
		$criteria->alias	=	'f';	
		$criteria->join	=	'INNER JOIN usr_user u ON (u.id = f.friend_id OR f.inviter_id = u.id) ';
		$criteria->addCondition("(f.friend_id = $my_id OR f.inviter_id = $my_id) AND u.id != $my_id AND u.username LIKE '%$keyword%'");
		$criteria->order	=	'f.requesttime DESC';
        ...
        public function getFriendShipStatus($me_id, $their_id){
            $row    =   YumFriendship::model()->find("(inviter_id={$me_id} AND friend_id={$their_id}) OR (inviter_id={$their_id} AND friend_id={$me_id})");
            if(!$row){
                return false;
            }else{
                return $row;
    ...
    public function getFriendIdFromRequest($me_id){
    	$rows    =   YumFriendship::model()->findAll("inviter_id={$me_id} AND `status` = " . YumFriendship::FRIENDSHIP_REQUEST);
    	if(!$rows){
    		return false;
    	}else{
    		$ids	=	array();
