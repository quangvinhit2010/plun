...
class YumUserController extends YumController {
	...
	public function accessRules() {
		return array(
				...
				array('allow',
					'actions'=>array('admin','delete','create','update', 'list', 'assign', 'generateData', 'csv', 'DeleteAvatar'),
	...
	public function actionView()
	{
		...
		$Elasticsearch->updateSearchIndexUser($model->id);		
	...
	public function loadAvatar($user)
	{
		if(!empty($user->avatar)){
	    	if(is_numeric($user->avatar)){
	    		...
	    		if(!empty($photo->id)){
	    			$url	=	$photo->getImageThumbnail160x160(true);
		    		return $url;
	    		} 
	    	}else{
		        $url = VAvatar::model()->urlAvatar($user->avatar);		
		        if(!empty($url))
				    return $url;
	    	}
    	}
    	return Yii::app()->request->hostInfo."/public/images/no-user.jpg";
	}
	
	public function actionDeleteAvatar(){
		if (Yii::app ()->request->isPostRequest && Yii::app ()->request->isAjaxRequest) {
			if (isset ( $_POST ['id'] )) {
				$model = $this->loadUser ( $_POST ['id'] );
				if(is_numeric($model->avatar)){
					$photo = Photo::model()->findByAttributes(array('id'=>$model->avatar, 'status'=>1));
					if(!empty($photo->id)){
						$photo->status = 0;
						$photo->save();
					}
				}else{
					$image = VAvatar::model()->urlAvatar($model->avatar);
