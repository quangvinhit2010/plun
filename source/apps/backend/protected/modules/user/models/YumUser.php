...
class YumUser extends YumActiveRecord
{
	...
	const REGISTER_STEP_COMPLETE = 0;
	const REGISTER_STEP_UPDATE_PROFILE = 1;
	const REGISTER_STEP_UPDATE_AVATAR = 2;
	const REGISTER_STEP_FINDHIM = 3;
	const REGISTER_STEP_SUGGEST = 4;
	
	const OPENID_TYPE_NONE = 0;
	const OPENID_TYPE_FACEBOOK = 1;
	const OPENID_TYPE_GMAIL = 2;
	...
	public function rules()
	{
		...
		$rules[] = array('username, createtime, lastvisit, lastpasswordchange, superuser, status, alias_name', 'required');
		$rules[] = array('notifyType, avatar, openid_identification', 'safe');
		$rules[] = array('password', 'required', 'on' => array('insert', 'registration'));
		$rules[] = array('salt', 'required', 'on' => array('insert', 'registration'));
		$rules[] = array('createtime, lastvisit, lastaction, superuser, status, is_vip, openid, openid_type, openid_register_complete', 'numerical', 'integerOnly' => true);
	...
	public function isFriendOf($invited_id)
	{
         ...
         * @param int $status
         */
        public function changeStatusFriendShip($inviter_id, $status){
            $conditions  =   array(
                'inviter_id' => $inviter_id,
                'friend_id' =>  $this->id
            );
            
            YumFriendship::model()->updateAll(
                    array('status' => $status), "inviter_id={$inviter_id} AND friend_id={$this->id}");
            
        }
        /**
         * get friends list
         * @param type $limit limit display
         * @param type $offset start offset
         * @return array 
         */
        public function getFriendlist($limit = 20, $offset = 0){
            $status =   YumFriendship::FRIENDSHIP_ACCEPTED;
            $CDbCriteria = new CDbCriteria();
            $CDbCriteria->alias =   's';
            $CDbCriteria->with  =   array(
                'inviter' => array(
                    'alias' => 'iv',
                    'joinType' => 'INNER JOIN',
                    'together' => true,
            ...
            $CDbCriteria->order = 's.requesttime DESC';  
            $CDbCriteria->addCondition("s.status='$status'", 'AND');
            $CDbCriteria->addCondition("s.inviter_id='{$this->id}' OR s.friend_id= '{$this->id}'", 'AND'); 
            
            $dbrow  =   YumFriendship::model()->findAll($CDbCriteria);
            
            //get total friends count
            $CDbCriteria = new CDbCriteria();
            $CDbCriteria->select =   'COUNT(*) AS total';
            $CDbCriteria->alias =   's';
            $CDbCriteria->with  =   array(
                'inviter' => array(
                    'alias' => 'iv',
            ...
            $CDbCriteria->addCondition("s.status='$status'", 'AND');
            $CDbCriteria->addCondition("s.inviter_id='{$this->id}' OR s.friend_id= '{$this->id}'", 'AND');     
            $total_friends  =   YumFriendship::model()->find($CDbCriteria);
            $total_friends  =   isset($total_friends->total)    ?   $total_friends->total   :   0;
            return array('dbrow' => $dbrow, 'total_friends' => $total_friends);            
        }
        public function getFriendships($status = 2, $limit = 20, $offset = 0)
	{
                ...
                $CDbCriteria->addCondition("s.status='$status'", 'AND');
                $CDbCriteria->addCondition("s.friend_id='{$this->id}'", 'AND');
                ...
                $CDbCriteria->addCondition("s.status='$status'", 'AND');
                $CDbCriteria->addCondition("s.friend_id='{$this->id}'", 'AND');     
                $total_request  =   YumFriendship::model()->find($CDbCriteria);
                $total_request  =   isset($total_request->total)    ?   $total_request->total   :   0;
                

                return array('dbrow' => $dbrow, 'total_request' => $total_request, 'friend_id' => $friend_id);
	}
