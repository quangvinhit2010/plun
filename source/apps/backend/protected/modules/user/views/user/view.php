...
if(Yii::app()->user->isAdmin()) {
	...
	array_push($attributes,
		...
		array(
			'name' => 'avatar',
			'type'=>'raw',
			'value' => CHtml::label(CHtml::link('Delete', 'javascript:DeleteAvatar('.$model->id.');', array('style' => 'width:160px; height:160px; position:absolute; display:block; text-align:center; top:45%;')).CHtml::image($this->loadAvatar($model)), '', array('id' => $model->id, 'style' => 'width:160px; height:160px; position:relative; display:block; text-align:center;')), 
			)
		);
...
<script type="text/javascript">
	...
	function DeleteAvatar(id){
		...
		if (answer && id){
			$.ajax({
				type: "POST",
				url: "/admin/user/user/deleteAvatar",
				data: { "id" : id },
				}).done(function() {
					$('label#' + id).remove();
			});
