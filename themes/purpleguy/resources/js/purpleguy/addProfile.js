...
var addProfile = {
	...
	join: function(){
		$('.menu .thamgia a').live('click', function(){	
			if($('#frm-addprofile').length > 0)
				$('#frm-addprofile')[0].reset();
...
//				    });
		...
		$('.but_upload').live('click', function(){
			$('#uploadImage2').hide();
			$('#uploadFile2').trigger( 'click' ); return false;
		});
		
		$('#frm-addprofile .but_submit').live('click', function(){
			$('#uploadImage2').hide();
			if($(".checkbox").is(':checked') == true){
				...
				if(arrFiles && arrFiles.length > 0){					
					...
					formdata.append("PurpleguyProfile[username]", $('#txtUsername').val());
					formdata.append("PurpleguyProfile[fullname]", $('#txtFullname').val());
					formdata.append("PurpleguyProfile[phone]", $('#txtPhone').val());
					formdata.append("PurpleguyProfile[email]", $('#txtEmail').val());
					
					$.ajax({
						url: $('#frm-addprofile').attr('action'),
						...
						success: function (res) {
							$('body').unloading();
							$('.popup_addProfile').find('.content').html('<ul><li><p>Bạn đã đăng ký thành công. Chúng tôi sẽ kiểm duyệt và đăng thông tin của bạn trong thời gian sớm nhất.<br/> Cám ơn và hy vọng bạn sẽ trở thành GƯƠNG MẶT ĐẠI DIỆN của PLUN.</p></li></ul>');
						}
					});
				}else{
					$('#uploadImage2').show();
					$('#uploadImage2').html('Bạn phải tải ảnh để tham gia chương trình !');
					$('body').unloading();
				}
			}else{
				console.log(5555);
		...
		$('#frm-addprofile .up_avatar .but_del').live('click', function(){
			var index;
