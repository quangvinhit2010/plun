<div class="content clearfix">
        <div class="left pic_person">
            ...
            <div class="btn_center request-button">
            	<?php if(!$request){ ?>
            		<a class="bg_btn" href="javascript:void(0);" title="Yêu Cầu Ghép Đôi" onclick="Tablefortwo.requestDating('<?php echo $profile->id; ?>');"><span class="bg_btn">Yêu Cầu Ghép Đôi</span></a>
            	<?php }else{ ?>
            		 <?php if($request): ?>
						 <?php if($request->inviter_id == $my_profile->id): ?>    
	            			<a class="bg_btn" href="javascript:void(0);" title="Đã Gởi Yêu Cầu"><span class="bg_btn">Đã Gởi Yêu Cầu</span></a>
						 <?php endif; ?>      
					 <?php endif; ?>
            	<?php } ?>
            </div>
