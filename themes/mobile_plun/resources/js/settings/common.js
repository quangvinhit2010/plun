...
function save_settings_basicinfo(){
    ...
    $.ajax({
        ...
        success: function(dt) {
            $(document).unloading();
	    	Util.popAlertSuccess('Data saved successfully!', 300);
            setTimeout(function () {
	         $( ".pop-mess-succ" ).pdialog('close');
	        }, 2000);   	
...
function saveMeasurement(){
	var measurement	= $('input[class=measurement]:checked').val();
	$(document).loading();
    var data = {
            'UsrProfileSettings[measurement]': measurement
    };
    $.ajax({
        type: "POST",
        data: data,
        url: '/settings/Savemeasurement',
        success: function(dt) {
	    	Util.popAlertSuccess(tr('Data saved successfully!'), 300);
	        setTimeout(function () {
	         $( ".pop-mess-succ" ).pdialog('close');
