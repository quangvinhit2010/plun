var Gab = {
    ...
    get_chat_history: function (jid){
    	$(".chat-messages").html('');
    	$.getJSON('/chat/history/jid/' + jid ,function(result){
    		var items = [];
    	    $.each(result, function(i, field){
    	    	items.push('<div class="chat-message">&lt;<span class="chat-name me">'+ Strophe.getNodeFromJid(field.fromJID)  +'</span>&gt;<span class="chat-text">'+ field.body +'</span></div>');
    	    });
    	    $(".chat-messages").html(items.join(''));
    	 });
    	
    	//<div class="chat-message">&lt;<span class="chat-name me">alex0511</span>&gt;<span class="chat-text">1</span></div>
...
$(document).ready(function () {
    ...
    $('.roster-contact').live('click', function () {
        ...
        if ($('#chat-' + jid_id).length === 0) {
            ...
            $('#chat-' + jid_id).append(
                "<div class='chat-messages'>" + history + "</div>" +
                "<input type='text' class='chat-input'>");
