...
var Photo = {
	delete_photo: function (item) {
        if(item){
        	var confirm = '<div class="ulti ulti-delete active">' +
						 ...
						 '</div>' +
						 '</div>';
        	$(item).closest('.item').append(confirm);
        	var photo_id = $(item).closest('.item').attr('id');
        	$(item).closest('.item').find('.ulti-delete .btn-gray').on( "click", function(e) {
        	//click(function(e){
        		e.preventDefault();
        		if($(this).attr('rel') == 'Yes'){
        			e.preventDefault();
        			var parent_list = $(item).closest('.item').closest('ul.item-list');
        			...
        			$.ajax({
        				...
        				url: '/photo/delete',
	 			        success: function(data){
	 			    		if($(data == 0)){
	 			    			//$(".pint-style .list").isotope('reloadItems').isotope({ sortBy: 'original-order'});
	 			    			/*$(item).closest('.item').fadeOut().remove();
	 			    			if(parent_list.find('li:last').css('display') != 'none'){
	 			    				var string = $(parent_list).attr('id');
	 		 			        	var list = string.replace('-view', '');
	 	 			    			Photo.show_more(list);
	 			    			}*/
	 			    			$(item).closest('.item').fadeOut().remove();
	 			    			$(this).unloading();
	 			    		} 
        			...
        			e.preventDefault();
        			$(item).closest('.item').find('.ulti-delete').hide();
        		}
        	});
        } else {
        	return false;
    ...
    photoAfterUpload: function (json, id){
    	...
    	if(json.success){
			...
			$("#"+id+"-upload ul.item-list-1").prepend('<li id="'+ json.fileid +'" class="item_1">'
						...
						+ '<img src="' + base_url + '/thumb275x275/' + json.filename +'" alt="" border="" width="160" height="160">'  
	...
	open_request_popup: function(cls, _this, row_key){
		$( ".popup-alert.photo-view-request .frame_content .list_photo_setting" ).html('');
		//var html = $(_this).closest('.photo-setting-request-list').find('li.'+ cls).find('ul.list_avatar li');
		var html	=	$('.' + row_key).find('li');
		...
		$.each(html, function(key, liobj){
			...
			if(src){
				request_data_html += '<li class="item_1 request-photo-'+rid+'">'
						+ '<a class="ava" title="" href="#">'
			                +'<img border="" data-pid="'+ rid +'" alt="" src="'+ src +'" width="160" height="160">'
			                + '<span class="ava-bg"></span>'
		                + '</a>'
		                + '<div class="info">'
			                + '<ul>'
				                + '<li>'
					                + '<div class="buttons">'
						                + '<button onclick="Photo.accept_photo_request(event, '+rid+', \''+cls+'\');" class="btn btn-violet">'+ tr('Accept')+'</button>'
						                + '<button onclick="Photo.decline_photo_request(event, '+rid+', \''+cls+'\');" class="btn btn-white">' + tr('Decline')+'</button>'
					                + '</div>'
				                + '</li>'
			                + '</ul>'
		                + '</div>'
			...
			if(key == 0){
				$(".popup-alert.photo-view-request").find('.right').hide();
			} else {
				$(".popup-alert.photo-view-request").find('.right').show();
			}
	    });
		$( ".popup-alert.photo-view-request .frame_content .list_photo_setting" ).attr('id', cls);
		$( ".popup-alert.photo-view-request .frame_content .list_photo_setting" ).html(request_data_html);
		sprScroll('.list_photo_setting');
		$( ".popup-alert.photo-view-request" ).dialog({
	...
	sharePhoto: function(obj){
		...
		$( ".photo_share_one" ).pdialog({
		    title: tr('Share Photo'),
		    width: 375,
		    height: 119
		});
	},
		...
		var data	=	{
		 ...
		 $.ajax({
		        ...
		        success: function(data){
		        	...
		        	if(data == '1'){
		        		$('.request-privatephoto' + photoid + ' p').text(tr('Request sent!'));
