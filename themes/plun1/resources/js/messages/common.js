...
var Messages = {
	...
	msgNewMessage: function () {		
		$(".title_message .right a").click(function (e) {
			$( ".popup-alert.new-messages" ).pdialog({
				title: tr('Send a message'),
				width: 600
			});
	...
	msgReply: function () {
		$(".replyMsg").on("keydown", function(event){
			var txtMsg = $(this);
			...
			if (event.which == 13 && !event.shiftKey && content.length > 0){
				...
				$.ajax({
				      ...
				      success: function( response ) {
				    	  txtMsg.closest('.message-detail').find('.message-list .mCSB_container').append(response);
	...
	msgDelete: function () {
		$(".news-message .btn-delete").unbind('click').bind('click', function(){
							  ...
							  click: function() {
								  if(delLink.length > 0){
										...
										$.ajax({
											...
											dataType: 'html',
											success: function( response ) {
												$('.news-message .feed-list-item .item').each(function(){
...
//												$('.feed-list-item').prepend(response);
												...
												$( ".popup-alert.deleteConversation" ).pdialog( "close" );
