...
Yii::app()->clientScript->registerScriptFile(Yii::app()->theme->baseUrl . '/resources/js/search/common.js', CClientScript::POS_BEGIN);
	...
	<div class="user">
		<div href="#" title="" class="photo">
			<div class="photo-wrap">
		        ...
		        $urlAvatar = $imageLarge = Yii::app()->createAbsoluteUrl('//public/images/'.Yii::app()->language.'/update-avatar.jpg');
		        
		        if(!empty($this->controller->usercurrent->avatar)){
					
		            if(is_numeric($this->controller->usercurrent->avatar)){
		                $photo = Photo::model()->findByAttributes(array('id'=>$this->controller->usercurrent->avatar, 'status'=>1));
		                /*
		                ...
		                */
		                ...
		                $urlAvatar = $imageLarge = VAvatar::model()->urlAvatar($this->controller->usercurrent->avatar);
		            }
	            ...
	            ?>
	                <a href="javascript:void(0);" onclick="$('#file').trigger( 'click' ); return false;"><img src="<?php echo $urlAvatar; ?>?t=<?php echo time();?>" width="150" alt="" border="" class="imgAvatar"/></a>
	                <a href="javascript:void(0);" class="btn-edit"><?php echo Lang::t('general', 'Edit Avatar')?></a>
	            <?php 
                }
		        ?>
                        ...
                        <div style="display:block;" class="edit_location check-in">
                            <div class="select_style check-in-country">
                            	<?php 
								    ...
								    echo CHtml::dropDownList('country_id',$country_id, $list_country_group, array('name' => 'txt-country', 'id' => 'txt-country', 'onchange' => "getStateCheckIn();",'class' => 'select-type-1 ci-country virtual_form'));                
                                ?>
                            ...
                            <div class="select_style check-in-state">
								<?php
									...
									echo CHtml::dropDownList('state_id',$state_id, $list_state_group, array('text' => 'text-check-in-state','name' => 'txt-state', 'id' => 'txt-state', 'onchange' => "getCityCheckIn();",'class' => 'select-type-1 ci-state virtual_form', 'empty' => Lang::t('search', '--Any--'))); 
								?>  
                                <span class="txt_select"><span class="text-check-in-state"><?php echo (isset($state['name'])) ? $state['name'] : ''; ?></span></span> <span class="btn_combo_down"></span>
                            </div>
                            <?php }else{ ?>
			...
			<div class="menu">
				<ul>	
				    <li>
						...
						<?php 
						echo CHtml::link('<i class="imed imed-message" title=""></i><span class="text">'.Lang::t('general', 'Message').'</span><span class="count" style="display: none;">0</span>', 
					...
					<li>
						...
						$photo_alert = 0;//SysPhotoRequest::getPhotoRequestUnreadCount($this->controller->usercurrent->id);
