...
var Photo = {
	...
	onUpload: function(id, name) {
		var listPhoto = Photo.activePhotoGroup.find('.content > ul');
		listPhoto.show();
		$('.empty_photo_list', Photo.activePhotoGroup).hide();
		$('> li:first-child', listPhoto).after('<li id="uploading-'+id+'" class="item uploading"><div class="meter"><span></span></div></li>');
		$('#uploading-'+id).data('step', 1);
        listPhoto.find('> li').boxResizeImg({
            parentWrap: listPhoto.parent('.content'),
            wRealBox: 160,
            hRealBox: 160
        });
    },
	delete_photo: function (item) {
        if(item){
        	...
        	var confirm = '<div class="ulti ulti-delete active show"><div class="c_item_box">' +
			 '<div class="_tempH"></div><div class="c_item"><p>' + tr('Delete this Photo?') + '</p>' +
			 '<div class="buttons">' +
        	...
        	var ultiDelete = parentItem.find('.ulti-delete');
        	ultiDelete.show();
        	parentItem.find('button.yes').click(function(){
        		var photoGroup = parentItem.closest('.photo-group');
        		var photo_id = $(item).closest('.item').attr('id');
    			var type = photoGroup.data('type');
    			parentItem.append('<div class="delete-loading"><div class="loadingInside"></div></div>');
    			ultiDelete.hide();
    			
    			var data = {photo_id: photo_id, type: type};
    			var listPhoto = parentItem.parent();
    			var limit = photoGroup.data('limit');
    			var currentItem = listPhoto.find('.item');
    			if(photoGroup.find('.pagging').length > 0) {
    				if(currentItem.length == limit) {
        				data.load_new = '1';
        			} else if(currentItem.length > limit) {
        				photoGroup.data('offset', Number(photoGroup.data('offset'))-1);
        			}
    			}
    			...
    			$.ajax({
    				...
    				url: '/photo/delete',
 			        success: function(data){
 			    		if(data.error == '0'){
 			    			parentItem.fadeOut().remove();
 			    			if(listPhoto.find('.item').length == 0) {
 			    				listPhoto.hide().prev().show();
 			    			}
 			    			if(data.after_save != '') {
 			    				var response = $(data.after_save);
 			    				currentItem.remove();
 			    				listPhoto.find('.add_photo').after(response.find('.item'));
 			    				var responsePagging = response.find('.pagging');
 			    				var currentPagging = photoGroup.find('.pagging');
 			    				if(responsePagging.length == 0)
 			    					currentPagging.remove();
 			    				else
 			    					currentPagging.attr('href', responsePagging.attr('href'));
                            }
                            listPhoto.find('> li').boxResizeImg({
                                parentWrap: listPhoto.parent('.content'),
                                wRealBox: 160,
    ...
    upload_photo_open: function(item){
    	Photo.activePhotoGroup = $(item).closest('.photo-group');
    	FineUploader_upload_photo._options.request.params.type = Photo.activePhotoGroup.data('type');
    ...
    photoBeforeUpload: function (id, name, loaded, total){
    	var uploadingItem = $('#uploading-'+id);
    	var loadingBar = uploadingItem.find('.meter span');
    	var percent = loaded / total * 100;
    	if(uploadingItem.data('step') == 1 && loaded == total) {
    		percent = '20%';
    	}
		uploadingItem.data('step', 2);
    ...
    photoAfterUpload: function (id, name, response){
        if(response.success) {
            var item = $(response.after_save).find('li.item');
            var groupPhoto = $('#uploading-'+id).closest('.photo-group');

            groupPhoto.data('offset', Number(groupPhoto.data('offset'))+1);

            $('#uploading-'+id).replaceWith(item);
            item.find('.icon_candy_photo').trigger('click');
            groupPhoto.find('.content > ul > li').boxResizeImg({
                parentWrap: groupPhoto.find('.content'),
                wRealBox: 160,
                hRealBox: 160
            });
            $('.sticky_column').fixed_col_scroll.callbackFun();
    ...
    photoOnError: function(id, name, errorReason){
        objCommon.unloading();
	    ...
	    $.ajax({
	        ...
	        success: function(data){
	    		if(data.success == '1'){
	    			...
	    			Util.popAlertFail(data.error, 330);
					setTimeout(function () {
						$( ".pop-mess-fail" ).pdialog('close');
					}, 2000);
	    		}
...
$(document).ready(function () {
    $(document).on('click','.photo_video .content ul li .share_del ol li a.icon_candy_photo',function(e){
        ...
        var divWrap = $('<div class="boxCandy_num show"><div class="wrap_set_num c_item_box"><div class="_tempH"></div><div class="c_item"><p>Nhập số candy cho ảnh</p><input class="candy-val" value="'+defaultCandy+'" /><a class="change-candy btn btn-gray" href="javascript:;">Submit</a><a class="cancel-change-candy btn btn-gray" href="javascript:;">Cancel</a></div></div></div>');
	...
	$(document).on('click', '.photo_video .pagging a', function(e){
		objCommon.loading();
		var self = $(this);
		...
		$.ajax({
	        url: url,
	        data: {offset: self.closest('.photo-group').data('offset')},
	        success: function(data){
	        	objCommon.unloading();
	        	var response = $(data);
	        	var parent = self.parent();
	        	
	        	parent.prev().after(response.find('li.item'));

                self.closest('.photo-group').find('.content > ul > li').boxResizeImg({
                    parentWrap: self.closest('.photo-group .content'),
                    wRealBox: 160,
                    hRealBox: 160
                });
	...
	$(document).on('click', '.cancel-change-candy', function(){
		$(this).closest('.item').removeClass('show-overlay')
		$(this).closest('.boxCandy_num').remove();
	});
	$(document).on('click', '.boxCandy_num .change-candy', function(){
		var self = $(this);
		var item = self.closest('.item');
		var price = self.prev().val();

		if(isNaN(price) || parseInt(price) !== Number(price)) {
			Util.popAlertSuccess(tr("Please enter only numeric characters ( No space and special characters )"), 600);
	        setTimeout(function () {
	        	$( ".pop-mess-succ" ).pdialog('close');
	        }, 2000);
			return false;
		} else if(Number(price) > Number($('#max-price').val()) || Number(price) < Number($('#min-price').val())) {
			var message = tr("the price of photo must be between &1 - &2.");
			message = message.replace('&1', $('#min-price').val());
			message = message.replace('&2', $('#max-price').val());
			Util.popAlertSuccess(message, 400);
	        setTimeout(function () {
	        	$( ".pop-mess-succ" ).pdialog('close');
	        }, 2000);
			return false;
		...
		self.closest('.boxCandy_num').hide();
		item.append('<div class="delete-loading"><div class="loadingInside"></div></div>');
		
        $.post( "/photo/setPrivatePhotoCandy", {photo_id: item.attr('id'), price: price}, function( response ) {
        	if(response.result == '1') {
        		item.data('candy', price).find('.candy-no').text(price);
        	} else {
                Util.popAlertSuccess(response.message, 600);
    	        setTimeout(function () {
    	        	$( ".pop-mess-succ" ).pdialog('close');
    	        }, 2000);
        	}
    		self.closest('.boxCandy_num').remove();
    		item.removeClass('show-overlay').find('.delete-loading').remove();
        }, 'json');
	});
	
	$('.photo_video').on('mouseenter', '.item', function(){
