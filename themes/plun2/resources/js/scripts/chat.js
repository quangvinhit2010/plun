...
setInterval(function(){
	$.post('/my/Checkonline', function(data){
...
$.fn.addChatBox = function(jid, callback, focus) {
	...
	if(listOffline.indexOf(jid) != -1) {
		$.post('/messages/UpdateUnread', {username: jid}, function(response){
...
var chat = {
	...
	attachEvent: function() {
		...
		$('#search-online-field-wrap input').keyup(function(e){
			...
			if(self.val() == '')
				...
				items.each(function(){
					if($('.name', this).html().toLowerCase().indexOf(self.val().toLowerCase()) == -1)
						$(this).hide();
					else
						$(this).show();
				});
		...
		$(document).on('click', '.quick-chat', function(e){
			if($('#chat-wrap').css('visibility') != 'visible')
				return false;
		...
		$(window).focus(function() {
			setCookie('windowIsActived', '1');
		});

		$(window).blur(function() {
			document.cookie = "windowIsActived=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/";
		});
	},
	connect: function() {
	...
	onMessage: function(message) {
		...
		if(message.find('composing').length > 0) {
			...
			if(inMessageSite && Messages.jid == jid) {
				var chatBox = $('#'+id);
				if(chatBox.length > 0) {
	...
	updateOnlineList: function(data) {
		...
		$.each(data, function(index, value) {
			...
			else if(value.photo_avatar != null)
	...
	updateOnlineListElasstic: function(data) {
		var html = '';
		if(data.status) {
			$.each(data.list, function(index, value){
				html += '<span id="cli-'+value.alias_name+'" class="online-user"><span class="avatar-wrap"><img src="'+value.avatar+'"></span><span class="name">'+value.alias_name+'</span></span>';
			});
		}
		$('#online-list .box-content-inside').html(html);
		$('#chat-box-list .box').each(function() {
	...
	playSound: function(){
		if(getCookie('windowIsActived') == '') {
			if(getCookie('playSound') == '') {
				setCookie('playSound', '1');
				$('#messageNotification').html('<audio autoplay="autoplay"><source src="/public/images/bing.wav" type="audio/mpeg" /><source src="/public/images/bing.ogg" type="audio/ogg" /><embed hidden="true" autostart="true" loop="false" src="/public/images/bing.wav" /></audio>');
				setTimeout(function(){
					document.cookie = "playSound=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/";
				}, 500);
			}
		}
    }
