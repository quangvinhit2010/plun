<?php if(isset($item)) {?>
	<?php foreach ($item as $rel=>$status) {?>
...
<li class="item">
	<div class="feed clearfix">
		<a href="<?php echo $photos['response_user_info']['url']; ?>" title=""
			class="ava">
		...
		<div class="info info_photo_setting">
			<p>
				<span class="username"><a
					href="<?php echo $photos['response_user_info']['url']; ?>"><?php echo $photos['response_user_info']['name']; ?></a></span>
			...
			<ul class="list_avatar">
				<?php foreach ($photos['response_list'] as $key => $photo) {?>
					<?php if($photo['status'] == SysPhotoRequest::REQUEST_ACCEPTED) { ?>
		                	<li><a rel="<?php echo $rel.$user_id ?>" href="<?php echo $photo['large_thumbnail_url'] ?>" class="group_response cboxElement">
